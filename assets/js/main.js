...
document.addEventListener("DOMContentLoaded", function (domLoadedEvent) {
	...
	for (let i = 0; i < anchorLinks.length; i++) {
		anchorLinks[i].addEventListener('click', function (e) {
			...
			let blockID = anchorLinks[i].hash;
			let target;
	 ...
	 * Sliders
	 */
	core_class.use([pagination, mousewheel, effect_fade, keyboard]);

	//about slider
	new core_class('.about__slider .slider__wrapper', {
		...
		on: {
			slideChangeTransitionStart: function (swiper) {
				swiper.slides.forEach(slide => {
					slide.classList.add('about__item--hidden');

					if (slide.classList.contains('swiper-slide-active')) {
						slide.classList.remove('about__item--hidden');
					}
				});
			},
		},
	...
	//portfolio slider
	new core_class('.portfolio__slider .slider__wrapper', {
		wrapperClass: 'slider__list',
		slideClass: 'slider__item',
		slidesPerView: 1,
		effect: "fade",
